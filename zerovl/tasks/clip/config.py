...
def task_cfg_init_fn(cfg : AttrDict):
    ...
    cfg.wandb.project = 'your_proj' # wandb的project
    cfg.wandb.entity = 'your_entity' # wandb的entity
    cfg.wandb.train_record_keys = ['loss', 'i2t_acc', 't2i_acc', 'lr'] # 需要往wandb里打的参数的key，这些参数需要能从batch_outputs中取到，一般不需改
    ...
    cfg.data.train_name = ['cc'] # 可到zerommml_datasets.py中查询有哪些可用 
    cfg.data.valid_name = ['f30k', 'coco'] # 可到zerommml_datasets.py中查询有哪些可用
    ...
    cfg.model.projection = AttrDict()
    cfg.model.projection.name = 'simple' #simple complex
    cfg.model.projection.dim = 512
    cfg.model.projection.text_projector_trainable = True
    cfg.model.projection.image_projector_trainable = True

    cfg.model.projection.complex_projection = AttrDict()
    cfg.model.projection.complex_projection.drop_out = 0.1
